...
foreach my $item (@{$items}) {
	...
	warn "AMMO $template->get_attr('template_code')";
	warn "AMMO $template->get_attr('template_id')";
	warn "AMMO $template->get_attr('top_text_margin')";

        my $labelJJG = C4::Labels::Label->new(
                                        batch_id            => $batch_id,
                                        item_number         => $item->{'item_number'},
                                        llx                 => $llx,
                                        lly                 => $lly+30,
	...
	if($template->get_attr('template_code') eq "BPARJJG_COTAS"){
		my $label_textJJG = $labelJJG->create_label();
		_print_text($label_textJJG) if $label_textJJG;
	}
